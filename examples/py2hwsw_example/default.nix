...
# Packages Py2HWSW so this example's Makefile can call the `py2hwsw` command
# directly, the same way any standalone Py2HWSW core repository does (see
# e.g. iob-hyperram-controller's own default.nix, which this mirrors).

{ pkgs ? import (fetchTarball "https://github.com/NixOS/nixpkgs/archive/25.05.tar.gz") {} }:

let
  py2hwsw_commit = "fd3aafcaaa798d556ced0f0f4e3611eaf0571857"; # Replace with the desired commit.
  py2hwsw_sha256 = "sha256-aBfrlfsn4b05gs2+0kW3xxL2OfOst+vvDZEdhps3xgM="; # Replace with the actual SHA256 hash.
  # Get local py2hwsw root from `PY2HWSW_ROOT` env variable, for a fast local
  # iteration loop that skips the GitHub fetch above entirely.
  ...
  extra_pkgs = with pkgs; [
    # Define other Nix packages this example needs here
  ];
