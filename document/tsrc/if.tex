...
\begin{lstlisting}[language=Verilog,caption={Standard control wires, present on virtually every unit.},label={lst:if_standard}]
input clk,
input rst,
...
\begin{lstlisting}[language=Verilog,caption={Data ports: one input/output pair per port index, plus optional \texttt{delay} and \texttt{done}.},label={lst:if_data}]
...
\begin{lstlisting}[language=Verilog,caption={Any wire not matching another interface's naming pattern is Config or State.},label={lst:if_config_state}]
input  [ANY_SIZE:0] anyOtherInputName,  // Config
output [ANY_SIZE:0] anyOtherOutputName, // State
\end{lstlisting}

As a concrete example, the whole of \texttt{Const} -- one of the simplest
built-in units, used throughout this document's examples -- is a single
Config input directly assigned to a data output:

\begin{lstlisting}[language=Verilog,caption={\texttt{hardware/src/units/Const.v}: a minimal unit exposing a single Config wire (\texttt{constant}) and a single data output (\texttt{out0}).},label={lst:const_unit}]
module Const #(
   parameter DATA_W = 32
) (
...
\label{sec:if_mem_mapped}

\begin{lstlisting}[language=Verilog,caption={Memory-mapped interface, following the IOb native interface convention.},label={lst:if_mem_mapped}]
input                 valid,
output                rvalid,
...
\begin{lstlisting}[language=Verilog,caption={Databus interface, indexed per instance (here, index 0).},label={lst:if_databus}]
...
\texttt{VRead} (Section~\ref{sec:address_gen}) is the built-in unit that
implements this interface to stream data in from RAM; its port declaration
shows the Databus and External Memory interfaces (below) side by side with
its own Config wires:

\begin{lstlisting}[language=Verilog,caption={\texttt{hardware/src/units/VRead.v}: excerpt of the port declaration.},label={lst:vread_ports}]
module VRead #(
   parameter DATA_W     = 32,
   ...
   // ... (address-generation Config wires omitted; see the Address Generation section)
...
\begin{lstlisting}[language=Verilog,caption={External Memory interfaces: two-port and true dual-port.},label={lst:if_ext_mem}]
// Two-port
